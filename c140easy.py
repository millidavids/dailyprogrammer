...
__question__ = """
    ...
    (CamelCase notation[4] or snake_case[5]).
    Your goal is to implement a program that takes an english-language series of words and converts them to a specific
    variable notation format. Your code must support CamelCase, snake_case, and capitalized snake_case.

    Input Description
    On standard console input, you will be given an integer one the first line of input, which describes the notation
    you want to convert to. If this integer is zero ('0'), then use CamelCase. If it is one ('1'), use snake_case. If
